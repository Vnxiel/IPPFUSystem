    ...
    function showLocDropdown() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const match = text.startsWith(filter); // 🔥 CHANGED THIS LINE
            items[i].style.display = match ? '' : 'none';
            if (match) anyVisible = true;
        }

        // Show only if there's input AND at least one match
        dropdown.style.display = (filter.length > 0 && anyVisible) ? 'block' : 'none';
        selectedIndex = -1;
    }

    function selectLoc(value) {
        input.value = value.trim(); // 💥 Trim any leading/trailing space
